...
export async function onRequestGet({ request, env }) {
  ...
  const members = await env.DB.prepare(`
    SELECT u.id, u.username, u.ingame_name, u.account_type, u.status, u.created_at, u.last_login,
  ...
  `).all();
  const ranks = await env.DB.prepare("SELECT id, name, slug, sort_order FROM guild_ranks WHERE lower(slug) NOT IN ('leader','deputy','officer','admin') ORDER BY sort_order").all();
